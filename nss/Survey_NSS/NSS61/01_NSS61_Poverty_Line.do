...
drop _merge
	...
	*Total rent and coveyance expenditure (impurws house, garage rent only for urban)
	egen double exp_rc_imputed = rowtotal(total_rent_imputed total_convy) //using rent_all instead of just house rent to keep consistency with the poverty measure
    ...
    *Rent and coveyance consumption per capita
	...
	gen double exp_rc_imp_pp = exp_rc_imputed/hh_size
	
	sum exp_rc_imp_pp exp_rc_pp exp_rc_al_pp [aw = pwt]
...
*Urban MPCE classes (copy from the method note, as the decile estimate from raw data is different)
gen mpce_10_cp = .
replace mpce_10_cp = 1 if mpce_mrp < 395
replace mpce_10_cp = 2 if inrange(mpce_mrp,395,485)
replace mpce_10_cp = 3 if inrange(mpce_mrp,485,580)
replace mpce_10_cp = 4 if inrange(mpce_mrp,580,675)

*Share of the rent and conveyance, fuel, cereal. 
foreach var in rc rc_al rc_imp fl cl dg {
gen exp_`var'_ratio = exp_`var'_pp/mpce_mrp *100 
}

table mpce_10 [aw = pwt], c(med exp_rc_al_ratio med exp_rc_ratio med exp_rc_imp_ratio med exp_fl_ratio med exp_cl_ratio) //(2.5/2.4), 11.6, 15.9 benchmark to rent 5.3, fuel 12.2, cereal 16.7 
table mpce_10 [aw = pwt], c(mean exp_rc_al_ratio mean exp_rc_ratio mean exp_rc_imp_ratio mean exp_fl_ratio mean exp_cl_ratio) 

table mpce_10_cp [aw = pwt], c(med exp_rc_al_ratio med exp_rc_ratio med exp_rc_imp_ratio med exp_fl_ratio med exp_cl_ratio) //(2.5/2.4), 11.6, 15.9 benchmark to rent 5.3, fuel 12.2, cereal 16.7 
table mpce_10_cp [aw = pwt], c(mean exp_rc_al_ratio mean exp_rc_ratio mean exp_rc_imp_ratio mean exp_fl_ratio med exp_cl_ratio) 

*Absolute exp. for rent and conveyance 
table mpce_10 [aw = pwt], c(med exp_rc_al_pp med exp_rc_pp med exp_rc_imp_pp med exp_fl_pp med exp_cl_pp) //(14.4/14)， 66, 91 at the MPCE class, benchmark to rent 30.68， fuel 70.4, cereal 96.5
