...
use "${raw}\b4_1",clear 
	...
	*hq_tenure_se: Secured Tenure: 
	...
	gen hq_tenure_se = 0  if (b4_10 != 6)  //only for household has dwelling.
	replace hq_tenure_se = 1 if inrange(b4_10,1,4) //with dwelling and the status is secured
    
	*hq_own: the dwelling is woned
	gen hq_own = (inrange(b4_10,1,2))
...
use "${raw}\b5",clear 
	...
	*hq_water_in: Distance to the principal source of drinking water: within dwelling/premises
	gen hq_water_in = 1 if inrange(b5_5,1,2)
	replace hq_water_in = 0 if inrange(b5_5,3,7)
...
use "${raw}\b6",clear
	...
	*hq_str_good: condition of structure good
	gen hq_str_good = (b6_4 == 1)
	
	*hq_elec: household has electricity for domestic use
	gen hq_elec = (b6_8 == 1)
	
	*hq_drainage: drainage system is pucca structure
	gen hq_drainage = inrange(b6_10,1,2)
...
use "${raw}\b7",clear
	...
	*hq_dwell_indi: type of dwelling is independent house
	gen hq_dwell_indi = (b7_1 == 1)
	
	*hq_room: total number of rooms in the dwelling
	gen hq_room = b7_2 + b7_3
	...
	*hq_pfloor: floor type is pucca: brick / stone / lime stone - 4,cement -5, mosaic / tiles - 6
	gen hq_pfloor = inrange(b7_14,4,6)
	...
	*hq_pwall: wall type is pucca: 
	...
	gen hq_pwall = inrange(b7_15,5,9)
	...
	*hq_proof: roof type is pucca
	...
	gen hq_proof = inrange(b7_16,5,9)
	
	*hq_rent_cost: if hired, the monthly rent (Rs.)
	gen hq_rent_cost  = b7_17 //86.37% missing (based on 3,4,5 in b4_10)
...
*****generate extra indicator*****
	...
	*cost of rent to umce
	gen hq_rent_rate = hq_rent_cost / hh_umce
