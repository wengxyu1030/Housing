...
preserve //best
    ...
    name(gr6) s(kr(40)) measure(Jaccard)
    tab gr6
...
use "${root}\Data Output Files\NSS76_All_clst_u.dta",replace
cluster kmeans in_floor_permanent in_roof_permanent in_wall_permanent san_flush san_imp_pit h20_piped_in h20_pump_in in_size_big in_sep_kitch, k(6) ///
    name(cluster) start(kr(40)) measure(Jaccard)
    tab cluster
    table cluster,c(mean in_floor_permanent mean in_roof_permanent mean in_wall_permanent mean san_flush mean san_imp_pit) format(%9.2f) center
	table cluster,c(mean h20_piped_in mean h20_pump_in mean in_size_big mean in_sep_kitch) format(%9.2f) center

cluster kmeans in_all_permanent san_flush san_imp_pit h20_piped_in h20_pump_in in_size_big in_sep_kitch, k(6) ///
    name(cluster_2) start(kr(42)) measure(Jaccard)
    tab cluster_2
    table cluster_2,c(mean in_all_permanent mean san_flush mean san_imp_pit) format(%9.2f) center
	table cluster_2,c(mean h20_piped_in mean h20_pump_in mean in_size_big mean in_sep_kitch) format(%9.2f) center

**order the cluster
//recode cluster (1 = 1)(2 = 5)(3 = 3)(4 = 6)(5 = 2)(6 = 4),gen(hq_cluster)
	
**distribution of slum and renter
gen hq_slum = 1-hq_nslum
tab cluster_2 hq_slum [aw = hh_weight],col
tab cluster_2 legal_rent [aw = hh_weight],col

**mean
table cluster_2 [aw = hh_weight],c(mean hq_slum med hh_umce mean legal_rent med cost_rent med in_ppl_area) format(%9.2f) center
