...
set.seed(46) #only using one variable for housing material:permanent structure
all_clst <- nss76 %>%
...
clst_table <- function(df){
  df %>%
    ...
    dplyr::select(cluster,hq_nslum,cost_rent,hh_umce,everything()) %>%
    ...
    rename("Cons (Rs.)" = hh_umce,
           ...
           "Rent (Rs.)" = cost_rent) %>%
    dplyr::select(-hq_nslum, -legal_own, -in_ppl_room)
}
...
##using roof instead of permanent structure for housing material.
nss76_clst_roof <- nss76 %>%
  dplyr::filter_at(vars(in_roof,ind,-hq_period),all_vars(!is.na(.)))%>%
  mutate(cluster = all_clst_roof$cluster)

result_roof <- nss76_clst_roof %>%
  mutate(across(c(in_roof,ind),as.factor)) %>%
  dplyr::select(in_roof,ind,cluster) %>%
  rename("Roof Material" = in_roof,
         "Separate Kitchen" = in_sep_kitch,
         "Number of Rooms" = in_room_grp,
         "Water Access" = h20,
         "Sanitation Access" = san) %>%
  group_by(cluster) %>%
  do(the_summary = summary(.))
result_roof$the_summary


clst_stat_roof <- clst_table(nss76_clst_roof)

clst_result_roof%>%
  cbind(clst_stat_roof) %>%
  arrange(`Rent (Rs.)`) %>%
  dplyr::select(cluster,everything())

##cross-check with k-means----
nss76_kmeans<- read.dta13(paste0(data_path,"/NSS76_urban_kmeans.dta")) %>%
  dplyr::select(cluster,id,b7_17)
skim(nss76_kmeans)

rerank_mode <-
  nss76_clst_roof %>%
  dplyr::select(id, cluster, b7_17) %>%
  group_by(cluster) %>%
  summarise(med_rent = median(b7_17, na.rm = TRUE)) %>%
  mutate(cluster_md = row_number(med_rent)) %>%
  right_join(nss76_clst_roof, by = "cluster")%>%
...
rerank_mean <- nss76_kmeans %>%
  ...
  group_by(cluster) %>%
  summarise(med_rent = median(b7_17, na.rm = TRUE)) %>%
  mutate(cluster_mn = row_number(med_rent))%>%
